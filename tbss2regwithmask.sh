...
this script expects mask files in the FA directory with the same name as the FA, e.g. Pat01_FA with the mask Pat01_FA_mask
masks should be one inside the brain and zero where the lesion is remember to delineating the lesion with a little bit of margin
...
do_reg(){
	...
	if [ ! -f ${o}_warp.msf ] ; then
	    echo $o
	    touch ${o}_warp.msf
	    echo "$FSLDIR/bin/fsl_reg $g $f ${g}_to_$f -e -FA -fnirt \"--inmask=${g}_mask\"" >> .commands
	fi
...
${FSLDIR}/bin/fsl_sub -T 60 -N tbss_2_reg -t .commands
